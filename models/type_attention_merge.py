...
from models.realation_fusion import attention, attention_output_shape, liter, liter_output_shape
from models.relation_base_model import BaseModel
from keras.layers import Concatenate, GRU, Dense, Dropout, Bidirectional, Lambda, Embedding, Input, Dot
import numpy as np
...
class Baseline(BaseModel):
    ...
    def __init__(self, max_len=125, class_num=9, use_development_set=True):
        ...
        self.name = "fusion_type_attention_merge"
    ...
    def build_model(self):
        ...
        tt = Lambda(liter,
                    ...
                    arguments={'length': self.max_len})(trigger_type)  # (?, 125, 50)
        et = Lambda(liter,
                    output_shape=liter_output_shape,
                    arguments={'length': self.max_len})(entity_type)  # (?, 125, 50)

        types = Concatenate()([tt, et])

        x1 = Concatenate()([types, sentence])  # (?, 125, 350)
        x1 = Dense(1)(x1)  # (?, 125, 1)
        x1 = Lambda(reduce_dimension,
                    ...
                    name='attention_1')(x1)  # (?, 125)
        x1 = Lambda(attention, output_shape=attention_output_shape, arguments={'dim': 600})([x1, sentence])  # (?, 600)

        # x2 = Concatenate()([et, sentence])
        # x2 = Dense(1)(x2)  # (?, 125, 1)
        # x2 = Lambda(reduce_dimension,
        #             output_shape=reduce_dimension_output_shape,
        #             arguments={'length': self.max_len},
        #             mask=self.sentence_embedding_layer.get_output_mask_at(0),
        #             name='attention_2')(x2)  # (?, 125)
        # x2 = Lambda(attention, output_shape=attention_output_shape, arguments={'dim': 600})([x2, sentence])  # (?, 600)

        # x = Concatenate()([x1, x2])
        x = Dropout(rate=0.5)(x1)
